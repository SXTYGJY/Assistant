...
for i, chunk in enumerate(chunks):
    ...
    embeddings_data.append(
        (chunk, str(vector))  # Doris ARRAY<FLOAT> 接受此格式
    )
...
# 连接到Doris
conn = mysql.connector.connect(**DORIS_CONFIG)
cursor = conn.cursor()

cursor.execute("USE doris_rag_test_db")


# 从数据库加载现有向量数据用于相似度比较
cursor.execute("SELECT content, embedding FROM doris_rag_demo")
...
duplicate_count = 0
similarity_threshold = 0.9  # 相似度阈值，超过此值认为是重复数据

for chunk, new_embedding in embeddings_data:
    is_duplicate = False

    # 与数据库中所有现有向量比较
    for existing_content, existing_embedding in existing_data:
        similarity = cosine_similarity(new_embedding, existing_embedding)
        if similarity > similarity_threshold:
            is_duplicate = True
            duplicate_count += 1
            print(f"⚠️  发现相似数据 (相似度: {similarity:.4f})")
            print(f"   新文本: {chunk[:100]}...")
            print(f"   现有文本: {existing_content[:100]}...")
            break

    if not is_duplicate:
        unique_embeddings_data.append((chunk, new_embedding))

print(f"✅ 语义去重结果 (阈值: {similarity_threshold}):")
print(f"   - 新增数据: {len(unique_embeddings_data)} 条")
print(f"   - 相似数据: {duplicate_count} 条 (已跳过)")

# 仅插入不重复的数据
if unique_embeddings_data:
    insert_sql = """
    INSERT INTO doris_rag_demo
    (content, embedding)
    VALUES (%s, %s)
    """
    cursor.executemany(insert_sql, unique_embeddings_data)
    conn.commit()
...
client = DorisVectorClient(database="doris_rag_test_db", auth_options=auth)

tbl = client.open_table("doris_rag_demo")

query = 'doris支持哪些存储模型？'
query_vec = embedding_model.embed_query(query)

result = tbl.search(query_vec, metric_type="l2_distance").limit(3).select(["id", "content"]).to_list()
